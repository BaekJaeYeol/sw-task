...
![그림 1. System Context Diagram](images/context.png)
...
| Revision Date | Version | Description | Author |
|--------------|----------|-------------|---------|
| 2026-06-02 | 1.00 | Initial Version | 백재열 |
...
Personal Assignment and Exam Schedule Management System은 대학생들이 과제와 시험 일정을 효율적으로 관리할 수 있도록 지원하기 위해 개발되는 시스템이다.

대학생들은 여러 과목의 과제 제출 일정과 시험 일정을 동시에 관리해야 한다. 그러나 일정이 많아질수록 중요한 마감일을 놓치거나 시험 일정을 잊어버리는 문제가 발생할 수 있다.

본 시스템은 과목별 과제 및 시험 일정을 등록하고 관리할 수 있도록 지원하며, 우선순위 설정 기능과 알림 기능을 제공하여 사용자가 효율적으로 학업 일정을 관리할 수 있도록 한다.

본 문서는 Analysis 단계에서 도출된 요구사항을 기반으로 시스템의 구조와 동작을 설계한 Design Document이다.

---

# 2. Class Diagram

아래 그림은 시스템의 Class Diagram이다.
...
과제 및 시험 일정의 공통 정보를 관리하는 부모 클래스이다.
...
아래 그림들은 주요 기능에 대한 Sequence Diagram이다.

## 3.1 Login

> Figure 2. Login Sequence Diagram

![Login Sequence Diagram](images/Sequence_Diagram1.png)

### Description

1. Student가 ID와 Password를 입력한다.
2. Login Screen이 Authentication Service를 호출한다.
3. Authentication Service가 Student Database를 조회한다.
4. 인증 성공 시 Dashboard를 출력한다.
5. 인증 실패 시 오류 메시지를 출력한다.

---

## 3.2 Add Assignment Schedule

> Figure 3. Add Assignment Sequence Diagram

![Add Assignment Sequence Diagram](images/Sequence_Diagram2.png)

### Description

1. Student가 과목을 선택한다.
2. 과제 정보를 입력한다.
3. Assignment Service가 Assignment 객체를 생성한다.
4. Database에 저장한다.
5. Calendar를 갱신한다.
...
> Figure 4. Mark Completed Sequence Diagram

![Mark Completed Sequence Diagram](images/Sequence_Diagram3.png)

### Description

1. Student가 일정을 선택한다.
2. Complete 버튼을 클릭한다.
3. Schedule Service가 상태를 Completed로 변경한다.
4. Database를 업데이트한다.
5. 완료 상태를 화면에 출력한다.
...
- 사용자는 직관적인 인터페이스를 통해 일정을 관리할 수 있어야 한다.
- 일정 등록 및 수정은 빠르게 수행되어야 한다.
- 데이터는 안정적으로 저장되어야 한다.
- 향후 기능 확장이 가능해야 한다.
...
| Term | Description |
|---------|------------|
| Class Diagram | 클래스와 관계를 표현하는 UML Diagram |
| Sequence Diagram | 객체 간 메시지 흐름을 표현하는 UML Diagram |
| State Machine Diagram | 시스템 상태 변화를 표현하는 UML Diagram |
| Assignment | 과제 일정 |
| Exam | 시험 일정 |
| Schedule | 일정 정보 |
| Notification | 알림 기능 |
